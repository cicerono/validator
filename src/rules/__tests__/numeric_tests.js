...
it("numeric should validate non-integer values when limits are non-integers", () => {
  ...
  expect(numeric("field", "42.5", {integerOnly: false, min: 42.1, max: 42.8})).toBe(null);
  expect(numeric("field", "9999999999.5", {integerOnly: false, min: 0, max: 9999999999.99})).toBe(
    null,
  );
});

it('numeric should return "numeric.integerOnly" for non-integer values when integerOnly is true', () => {
...
it("should validate minimal decimal points", () => {
  ...
  expect(numeric("field", 12.12, {integerOnly: false, decimalPlaces: {min: 3}})).toBe(
    "numeric.decimalPlaces.min",
  );
  expect(numeric("field", 12, {integerOnly: false, decimalPlaces: {min: 1}})).toBe(
    "numeric.decimalPlaces.min",
  );
...
it("should validate maximal decimal points", () => {
  ...
  expect(numeric("field", 12.12, {integerOnly: false, decimalPlaces: {max: 1}})).toBe(
    "numeric.decimalPlaces.max",
  );
